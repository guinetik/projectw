var app = angular.module('wats.app', ["LocalStorageModule", "emoji"]);

app.controller('AppCtrl', ['$scope', 'auth', '$location', '$ionicNavBarDelegate', function ($scope, auth, $location, $ionicNavBarDelegate) {
    auth.checkUserCredentials(true);
...
app.controller('ChatsCtrl', ['$scope', 'chats_service', 'auth', function ($scope, chats_service, auth) {
    //initials
    $scope.showSearch = false;
    $scope.chats = [];
    if (auth.checkUserCredentials(true)) {
        //populate list
        chats_service.getLatestChats(function (data, error) {
            if (error) {
...
app.controller('ChatCtrl', ['$scope', 'chats_service', 'auth', '$ionicScrollDelegate', '$stateParams', '$timeout', '$ionicTabsDelegate', '$ionicModal', '$window', function ($scope, chats_service, auth, $ionicScrollDelegate, $stateParams, $timeout, $ionicTabsDelegate, $ionicModal, $window) {
    this.name = "ChatCtrl";
    $scope.msg = '';
    //emoji modal
    $ionicModal.fromTemplateUrl('templates/emoji.html', {
        scope: $scope,
    ...
    $scope.openEmoji = function () {
        $scope.modal.show();
        angular.element(".modal-backdrop").css("height", ($window.innerHeight - 90).toString() + "px");
    };
    $scope.closeEmoji = function () {
        $scope.modal.hide();
        $scope.msg += " ";
        var textarea = document.getElementById('txt_msg');
    ...
    if (auth.checkUserCredentials(false)) {
        chats_service.getChatMessages($stateParams.id, function (data, error) {
            if (error) {
                console.log("error:", error);
            }
            $scope.messages = data;
        ...
        $scope.add = function () {
            chats_service.sendMessage($stateParams.id, $scope.msg, function (data, error) {
                if (error) {
                    console.log("error:", error);
                }
...
app.controller('ContactsCtrl', ['$scope', '$stateParams', 'auth', '$ionicPopup', function ($scope, $stateParams, auth, $ionicPopup) {
    auth.checkUserCredentials(false);
    $scope.showSearch = false;
    $scope.showAddUserPopup = function () {
        var myPopup = $ionicPopup.show({
            template: '<input type="tel" ng-model="user.number">',
            title: 'Add contact',
            subTitle: 'Enter you contact number',
            scope: $scope,
            buttons: [
                { text: 'Cancel' },
                {
...
app.controller('SettingsCtrl', ['$scope', '$stateParams', 'auth', function ($scope, $stateParams, auth) {
    auth.checkUserCredentials(false);
}]);

app.controller('ProfileCtrl', ['$scope', '$stateParams', 'auth', function ($scope, $stateParams, auth) {
    auth.checkUserCredentials(false);
}]);

app.controller('LoginCtrl', ['$scope', '$stateParams', 'auth', function ($scope, $stateParams, auth) {
    $scope.user = {};
...
app.controller('EmojiCtrl', ['$scope', '$stateParams', function ($scope, $stateParams) {
    var emojis = [
    ...
    var emojis_earth = ["sunny", "umbrella", "cloud",
    ...
    var emoji_tools = ["fireworks", "sparkler", "wind_chime", "rice_scene", "jack_o_lantern",
    ...
    var emoji_city = ["house", "house_with_garden", "school", "office", "post_office", "hospital",
    ...
    var emoji_signs = ["one", "two", "three", "four", "five", "six", "seven",
    ...
    $scope.toggleEarth = function () {
        $scope.emojis = emojis_earth;
    };

    $scope.toggleTools = function () {
        $scope.emojis = emoji_tools;
    };

    $scope.toggleCity = function () {
        $scope.emojis = emoji_city;
    };

    $scope.toggleSigns = function () {
        $scope.emojis = emoji_signs;
    };
}]);